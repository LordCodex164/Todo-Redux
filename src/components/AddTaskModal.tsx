        ...
        <div className='bg-[#fff] rounded-[10px] absolute top-[10em] left-[18em] flex flex-col min-w-[450px] mx-auto min-h-[153px] px-[16px] shadow-md pt-[16px]'> 
          <div className='flex justify-between mb-[10px]'>
              <div className='flex flex-col gap-[20px]'>
                <input onChange={(e) => setFormData({...formData, taskName: e.target.name})} type="text" value={formData.taskName} placeholder='Task Name' className='o outline-none'/>
                <input onChange={(e) => setFormData({...formData,  desc: e.target.name})} type="text" value={formData.desc} placeholder='Description' className='o outline-none'/>
              </div>
          ...
          <div className='flex flex-col'>
           ...
           <Listbox value={selected} onChange={setSelected}>
             ...
             <div className='relative mt-3'>
                <Listbox.Button className="relative w-full flex justify-between cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                ...
                <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                   {buttonStatus.map((item, i) => (
                    ...
                    >
                      ...
                      {({selected}) => (
                        <>
                         <span className={`block ${selected ? "font-medium text-[#fff]": "font-normal"}`}>
                            {item.name}
                         </span>
                        </>
                      )}
          ...
          <div className='flex flex-col items-start justify-start mt-[30px]'>
            ...
            <Button btnType='Date' key={title} title={title} btnStyles='min-w-[88.72px] mt-[10px] h-[28px] rounded-sm border-[2px] border-[#c8f4ee]'/>
          ))}
          </div>
